...
export const useFakeStackOverflow = (app: ApplicationInterface) => {
    ...
    const setQuestionPage = (pageIndex = 0, search = "", title = "All Questions") => {
        console.log("Updating question page with:", { pageIndex, search, title });
    ...
    const handleAnswer = (questionId: string) => {
        setQid(questionId);
        const newParams = { ...params, qid: questionId };
        setPageInstance(getPage({ pageName: "answer", params: newParams }));
    };
    ...
    const clickTag = (tname: string) => {
        ...
        // Set search query to filter questions by the selected tag
        setSearch(`[${tname}]`);
    
        // Ensure the page displays the filtered questions in newest order
        setQuestionOrder("Newest");
    
        // Navigate to the home page (which displays filtered questions)
        // setPageInstance(getPage({ pageName: "home", params }));
        setPageIndex(0);
    };
    ...
    useEffect(() => {
        console.log("Updating pageInstance to show filtered questions");
        setPageInstance(getPage({ pageName: "home", params }));
        // eslint-disable-next-line react-hooks/exhaustive-deps
    }, [search, questionOrder, pageIndex]);   
